...
<table width="100%" >
	<tr>
		...
		<td width="100%" class="ver01" >
				...
				<form action="<c:url value='/sys/org/search'/>" id="queryForm" method="post">
				<input id="CSRFToken" name="CSRFToken" type="hidden" value=""  />
				<input type="hidden" id="parentId" name="parentId" value="${parentId}"/>
			<shiro:hasPermission name="SafeM::search">
 ...
 <script type="text/javascript">
	...
	function initGrid() {
		grid = $("#dataBasic").quiGrid({
			columns:[
				...
				{ display: '名称', name: 'title',     align: 'left', width: "20%"},
				{ display: '开始日期', name: 'beginTime',     align: 'center', width: "10%"},
				{ display: '安全天数', name: 'days',     align: 'right', width: "5%"},
			    { display: '简介', name: 'brief', 	 align: 'left', width: "20%"},
			    { display: '显示', name: 'allowed', 	 align: 'center', width: "5%",
				 render : function(rowdata, rowindex, value, column){ return renderShow(rowdata,value);}},
           		{ display: '操作', isAllowHide: false, align: 'center', width:80,
						 render: function (rowdata, rowindex, value, column){
	                 	    return '<div class="padding_top4 padding_left5">'
	                                  + '<shiro:hasPermission name="SafeM:edit"><span class="img_edit hand" title="修改" onclick="onEdit(' + rowdata.id + ')"></span></shiro:hasPermission>' 
	                                 + '<shiro:hasPermission name="SafeM:delete"><span class="img_delete hand" title="删除" onclick="onDelete(' + rowdata.id+','+rowindex + ')"></span></shiro:hasPermission>'
	                             + '</div>';
		                 }
		            }
			  ],
		 url: "<c:url value='/cms/safe/table'/>", sortName: 'orderNum',rownumbers:true,checkbox:true,
         height: '100%', width:"100%",pageSize:10,percentWidthMode:true,
        
         toolbar:{
        	 items:[
        		  <shiro:hasPermission name="SafeM:save">
	...
	//渲染节点
	function renderShow(rowdata,value){
		if(value=="y"){
           return '<div class="padding_top4 padding_left5"><span class="img_lightOn hand" title="显示 " onclick="onShow(' + rowdata.id + ')"></span></div>';
       	}
        if(value=="n"){
          return '<div class="padding_top4 padding_left5"><span class="img_light hand" title="关闭" onclick="onShow(' + rowdata.id + ')"></span></div>';
        }
	...
	function addUnit() {
		...
		if(orgid != null && orgid != ""){
			if(selectTreeNode.menuType=="y"){
				...
				$.post("<c:url value='/cms/safe/create'/>",
		  			{"orgid":orgid,"dicid":"10","CSRFToken":$("#CSRFToken").val()},
		  			function(result){
		  				getToken();
		  				handleResult(result);
					},"json");
				...
				top.Dialog.alert("请选择一个部门",null,null,null,2);
		}
	}
	...
	function onEdit(rowid){
		...
		top.Dialog.open({
			URL:"<c:url value='/cms/safe/preEdit?safeId='/>" + rowid+"&url=cms/safe/DayEdit",
			Title:"修改",Width:500,Height:300});
	}
	
	//删除	
	function onShow(rowid){
		
		  	//删除记录
		  	$.post("<c:url value='/cms/safe/ifshow'/>",
