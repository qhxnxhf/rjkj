...
<table width="100%" >
		...
		<td width="100%" class="ver01" >
			...
			<table width="100%">
				<tr>
					...
					<td>状态：</td>
					<td>
						<select  name='status'   data='{"list":[{"value":"1","key":"编辑"},{"value":"2","key":"待审"},{"value":"3","key":"待发"},{"value":"4","key":"发布"},{"value":"5","key":"废止"}]}' selWidth="120" ></select>
 ...
 <script type="text/javascript">
	...
	function initGrid() {
		...
		isScroll: true, 
		 url: "<c:url value='/cms/pub/checktable'/>", sortName: 'id',rownumbers:false,checkbox:true,
	...
	function onMainOper(rowdata){
		var sss='<div class="padding_top4 padding_left5">';
			if(rowdata.status==2){
				sss=sss + '<shiro:hasPermission name="CheckM:auth"><span class="img_ok hand" title="审核通过" onclick="onAuth1(' + rowdata.id + ')"></span></shiro:hasPermission>' 
	                    + '<shiro:hasPermission name="CheckM:auth"><span class="img_no hand" title="审核否决" onclick="onAuth2(' + rowdata.id + ')"></span></shiro:hasPermission>';
	...
	function onAuth1(rowid){
		top.Dialog.confirm("确定通过审核吗？",function(){
		  	var shyj=$("#shyj").val();
		  	$.post("<c:url value='/cms/pub/authNews'/>",
		  			{"ids":rowid,"zt":3,"shyj":"同意！"+shyj,"CSRFToken":$("#CSRFToken").val()},
		  			function(result){
		  				grid.loadData();
					},"json");
					//刷新表格数据 
	...
	function onAuth2(rowid){
		top.Dialog.confirm("确定退回吗？",function(){
		  	var shyj=$("#shyj").val();
		  	$.post("<c:url value='/cms/pub/authNews'/>",
		  			{"ids":rowid,"zt":1,"shyj":"否决!"+shyj,"CSRFToken":$("#CSRFToken").val()},
		  			function(result){
		  				grid.loadData();
					},"json");
					//刷新表格数据 
