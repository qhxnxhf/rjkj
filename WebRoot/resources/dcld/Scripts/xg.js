...
(function () {
    ...
    var _editor = _xg.Editor = function (containerId, initStyles) {
        ...
        thisEditor.initStyles = "html{overflow:hidden;}body{margin:0;padding:20px;text-align:center;}table{border-collapse:collapse;border-spacing:0;}" + (initStyles ? initStyles : "");//html溢出隐藏是为了取消默认滚动条

        thisEditor.$iframe = $("<iframe/>").attr("id","xg-editor-iframe-"+containerId).attr("name","xg-editor-iframe-"+containerId).attr("frameborder", "0");//添加frameborder是为了兼容ie7/ie8
    ...
    _editor.prototype = {
        render: function () {
            ...
            thisEditor.$container.append(thisEditor.$iframe);//alert(thisEditor.containerId);
            if($.browser.msie&&($.browser.version==7||$.browser.version==8)){
	            thisEditor.$iframe.load(function () {
	                var $iframeDoc = $(document.getElementById("xg-editor-iframe-"+thisEditor.containerId).contentWindow.document);//thisEditor.$iframe.contents();
	            ...
	            $(window.frames["xg-editor-iframe-"+thisEditor.containerId]).ready(function () {
	                var $iframeDoc = $(document.getElementById("xg-editor-iframe-"+thisEditor.containerId).contentWindow.document);//thisEditor.$iframe.contents();
	                if ($iframeDoc.find("head").length < 0) { $("<head/>").appendTo($iframeDoc); }
	                if ($iframeDoc.find("body").length < 0) { $("<body/>").appendTo($iframeDoc); }
